...
const BlogComponent = ({ creator, data, tags, edit, title }) => {
  ...
  useEffect(() => {
    ...
    if (creator) {
      axios
        .get(`http://localhost:3001/api/user/${creator}`)
          ...
          {tags
            ? tags.map((obj, index) => {
                return (
                  <div
                    key={index}
                    className="border border-black font-light px-1 mr-2 my-1 rounded-md"
                  >
                    {obj}
                  </div>
                );
          ...
          <div>
            ...
            {parsed.map((obj, index) => {
              switch (obj.type) {
                ...
                case "ahref":
                  return (
                    <a key={index + 5} href={obj.link} alt={obj.info}>
                      {obj.info}
                    </a>
                  );
                case "code":
                ...
                case "heading":
                  switch (obj.level) {
                    case 1:
                      return (
                        <div className="text-6xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                    case 2:
                      return (
                        <div className="text-5xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                    case 3:
                      return (
                        <div className="text-4xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                    case 4:
                      return (
                        <div className="text-3xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                    case 5:
                      return (
                        <div className="text-2xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                    case 6:
                      return (
                        <div className="text-xl font-bold" key={index + 5}>
                          {obj.title}
                        </div>
                      );
                  }

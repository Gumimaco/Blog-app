    ...
    <div>
      {viewUser ? (
        <div className="bg-white rounded-md m-4 flex-col flex items-center p-4 border ">
          <GetImage
            classes="profile-picture h-32 w-32"
            image={viewUser.profile_picture}
          />

          <div className="text-2xl font-bold">{viewUser.username}</div>
          {viewUser.description ? <div>{viewUser.description}</div> : null}
          {user && user._id === id ? (
            <button onClick={redirectToSettings} className="custom-button h-6">
              Edit profile
            </button>
          ) : null}
        </div>
      ...
      {posts ? (
        <div className="flex flex-col">
          ...
          {posts
            ...
            .map((obj, index) => {
              return (
                <BlogCard classes="mx-4" blog={obj} key={index} user={user} />
              );
            })}
        </div>
      ) : null}
